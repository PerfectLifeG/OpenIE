任务描述：我会给你一条句子和一个分类集合（coarse_types），你需要根据这个分类集合抽取句子中的名词，抽取是，名词的类型应该是分类集合中的任意类型。然后对抽取到的名词进行更加细节的分类（fine_type），要求 fine_type 决不能为空或 "unknown"，必须要判断一个最相近的细分类，并且要使用与输入相同的语言。最终以 json 格式返回。 
